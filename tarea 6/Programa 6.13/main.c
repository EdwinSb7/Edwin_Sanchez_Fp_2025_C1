...
const int MES = 12;  // Meses
const int DEP = 3;   // Departamentos
const int AÑO = 8;   // Años
...
{
    /* Esta función se utiliza para leer un arreglo tridimensional de tipo
...
void Funcion1(float A[][DEP][AÑO], int F, int C, int P)
{
    ...
    for (I = 0; I < F; I++)
        for (J = 0; J < C; J++)
            SUM += A[I][J][1];  // Segundo año (índice 1)
...
void Funcion2(float A[][DEP][AÑO], int F, int C, int P)
{
    ...
    for (I = 0; I < F; I++)
    {
        for (J = 0; J < C; J++)
        {
            switch (J)
            {
                case 0: SUM1 += A[I][J][P - 1]; break;  // Departamento 1: Hilos
                case 1: SUM2 += A[I][J][P - 1]; break;  // Departamento 2: Licra
                case 2: SUM3 += A[I][J][P - 1]; break;  // Departamento 3: Lanas
            }
        }
    }

    // Comparación de los departamentos con mayores ventas
