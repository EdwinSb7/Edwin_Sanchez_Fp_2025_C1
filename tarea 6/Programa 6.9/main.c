...
void main(void)
{
    ...
    while (TAM > 10 || TAM < 1);  /* Verifica que TAM esté en el rango correcto */
...
/* Esta funcion se utiliza tanto para calcular el módulo entre el índice del
...
/* Esta funcion obtiene el resultado de una sumatoria. */
{
    int I;
    float SUM = 0.0;
    for (I = 0; I < N; I++)  /* Corregi el rango del ciclo */
    {
        SUM += A[N][I];
    }
    return (SUM);
...
/* Esta funcion obtiene el resultado de la productoria. */
{
    int I;
    float PRO = 1.0;
    for (I = N; I < M; I++)  /* Corregi el rango del ciclo */
    {
        PRO *= A[I][N];
    }
    return (PRO);
